...
class MsalViews:
    def __init__(self, azure_identity_web):
        ...
        self.prefix = self.azure_identity_web.aad_config.django.auth_endpoints.prefix + "/"
        self.endpoints = self.azure_identity_web.aad_config.django.auth_endpoints
    ...
    def sign_in(self, request):
        ...
        auth_url = self.azure_identity_web.get_auth_url(redirect_uri=request.build_absolute_uri(reverse(self.endpoints.redirect)))
    ...
    def edit_profile(self, request):
        ...
        auth_url = self.azure_identity_web.get_auth_url(
                redirect_uri=request.build_absolute_uri(reverse(self.endpoints.redirect)),
                b2c_policy=self.azure_identity_web.aad_config.b2c.profile)
        return redirect(auth_url)
    ...
    def aad_redirect(self, request):
        ...
        return self.azure_identity_web.process_auth_redirect(
            redirect_uri=request.build_absolute_uri(reverse(self.endpoints.redirect)),
    ...
    def sign_out(self, request):
        ...
        return self.azure_identity_web.sign_out(request.build_absolute_uri(reverse(self.endpoints.post_sign_out)))    # send the user to Azure AD logout endpoint
    ...
    def post_sign_out(self, request):
        ...
        self.azure_identity_web.remove_user(request.identity_context_data.username)  # remove user auth from session on successful logout
